...
export default function WebVoiceAgent({ apiKey }: WebVoiceAgentProps) {
  ...
  const startRecording = async () => {
    if (Platform.OS !== 'web') {
      Alert.alert('Platform not supported', 'This component is designed for web use');
      return;
    }
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (apiKey) {
          ...
          setResponse('I heard you say: Hello! This is a web demo recording. How can I help you today?');

          // Use Web Speech API for TTS if available
          if ('speechSynthesis' in window) {
            const utterance = new SpeechSynthesisUtterance('I heard you say: Hello! This is a web demo recording. How can I help you today?');
      ...
      Alert.alert('Recording Error', 'Failed to access microphone. Please ensure microphone permissions are granted.');
    }
  };
  ...
  const handlePress = () => {
    if (!isSupported) {
      Alert.alert('Not Supported', 'Web Audio API is not supported in this browser');
      return;
    }
  ...
  return (
    <View className="flex-1 justify-center items-center p-6 space-y-6">
      <Text className="text-3xl font-bold text-gray-800 mb-4">Web Voice Assistant</Text>

      {!isSupported && (
        <View className="bg-yellow-100 p-4 rounded-lg mb-4">
          <Text className="text-yellow-800 text-center">
            Web Audio not supported in this browser. Please use Chrome, Firefox, or Safari.
          </Text>
        </View>
      )}
      ...
      >
        <Text className={`text-lg font-semibold ${!isSupported ? 'text-gray-500' : 'text-white'}`}>
          {!isSupported
            ? 'Not Supported'
            : isProcessing
              ? 'Processing...'
